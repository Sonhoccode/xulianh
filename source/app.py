...
# ================== CONFIG ==================
MODEL_PATH = r"runs/detect/YOLO_Stage2/weights/last.pt"   # UPDATE theo model của bạn
# MODEL_PATH = r"runs/detect/YOLO_Stage22/weights/best.pt"   # UPDATE theo model của bạn
...
class TrafficSignApp:
    ...
    def display_result(self, frame, label_widget, text_widget):
        ...
        if len(result_obj.boxes) == 0:
            ...
            text_widget.insert("1.0", f"Tìm thấy {len(result_obj.boxes)} biển báo:\n------------------------------\n")
    ...
    def cam_loop(self):
        while self.is_cam_running:
            ...
            if not ret:
                break
            self.root.after(0, self.display_result, frame, self.cam_label, self.text_cam)
            time.sleep(0.03)
    ...
    def video_loop(self):
        while self.is_vid_running:
            ...
            if not ret:
                break
            self.root.after(0, self.display_result, frame, self.vid_label, self.text_vid)
            time.sleep(0.03)
        self.is_vid_running = False
